defmodule JaSerializer.Builder.Relationship do
  ...
  defp filter_fields(relationships, context = %{serializer: serializer, opts: opts}) do
    case opts[:fields] do
      fields when is_map(fields) -> do_filter(relationships, fields[serializer.type(context.data, context.conn)])
      _any -> relationships
    end
  end
  defp filter_fields(relationships, _), do: relationships
